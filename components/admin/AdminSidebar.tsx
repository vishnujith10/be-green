...
export default function AdminSidebar() {
  ...
  return (
    <>
      {/* ================= Desktop Sidebar ================= */}
      <aside className="hidden md:flex fixed left-0 top-0 h-screen w-64 bg-[#0F1A12] text-white flex-col z-50">
        {/* Logo */}
        <div className="px-6 py-6 border-b border-white/10">
          ...
          >
            ...
            <div>
              ...
              <p className="text-[10px] uppercase tracking-[0.2em] text-emerald-400/80 font-medium">
                Admin Panel
              </p>
            </div>
          </Link>
      ...
      {/* ================= Mobile Header ================= */}
      <header className="md:hidden fixed top-0 left-0 right-0 z-50 bg-[#0F1A12] border-b border-white/10">
        <div className="px-4 h-16 flex items-center justify-between">
          ...
          <button
            onClick={handleLogout}
            className="flex items-center gap-2 px-3 py-2 rounded-lg bg-red-500/10 text-red-400 text-sm font-medium"
          >
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.label}
                  href={item.href}
                  className={`flex-1 flex items-center justify-center gap-2 rounded-xl py-3 text-sm font-medium transition-all ${isActive
